...
var joinRoom = function(socket, io, room) {
  ...
  io.sockets.in(room).emit('startProgress', {
...
var handleProgress = function(socket, io) {
...
var handleDisconnection = function(socket, io) {
  socket.on('disconnect', function() {
    ...
    io.sockets.in(leavingRoom).emit('message', {
      text: (nicknames[socket.id] + " has left " + leavingRoom + "."),
      room: leavingRoom
    })
...
var socketIOListen = function(server){
    ...
    joinRoom(socket, io, "Type Type Meow");
    handleMessages(socket, io);
    handleProgress(socket, io);
